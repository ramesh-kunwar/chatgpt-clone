"use client";
import { SidebarClose, SidebarCloseIcon, SidebarOpenIcon } from "lucide-react";
...
export default function SidePanel() {
  ...
  return (
    <div
      className={clsx(
        "bg-gray-50 transition-all duration-150 flex flex-col box-border h-[100vh] overflow-scroll border-r border-gray-200",
    ...
    >
      <div className="sticky top-0 z-1 bg-gray-50">
        ...
        <div className="px-2">
          <Link
            href="/"
            className="px-2 rounded-sm flex gap-3 py-2 text-md w-full hover:bg-gray-200"
          >
            ...
            {/* {chats.map((element: any) => {
              return (
                <Link
                  key={element.id}
                  href={`/c/${element.id}`}
                  className="block p-2 rounded-sm text-md hover:bg-gray-200 truncate"
                >
                  {element.title}
                </Link>
              );
      ...
      <div className="sticky bottom-0 bg-gray-50 flex-0 inset-0 mt-auto border-t border-gray-200 p-2">
        <div className="flex items-center justify-between">
          ...
          >
            <button className="px-2 py-1 bg-white rounded-2xl border border-gray-300 outline-0 text-xs font-medium cursor-pointer">
              Upgrade
            </button>
          </div>
        </div>
